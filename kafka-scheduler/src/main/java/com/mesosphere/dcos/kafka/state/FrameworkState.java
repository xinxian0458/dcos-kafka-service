...
public class FrameworkState implements TaskStatusProvider {
    ...
    public FrameworkID getFrameworkId() {
        Optional<FrameworkID> frameworkIDOptional = stateStore.fetchFrameworkId();
        if (frameworkIDOptional.isPresent()) {
            return frameworkIDOptional.get();
        } else {
            log.warn("No FrameworkID returned. This is expected when the service is starting for the first time.");
            return null;
        }
    }
